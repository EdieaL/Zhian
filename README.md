...
<p align="center">
  <img src="assets/vulnrepair-pipeline-en.png"
       alt="VulnRepair Technical Pipeline"
       width="100%">
</p>
...
Zhian is not invoked for every task. It is used only when the DeepSeek V4 Flash pipeline fails to pass validation within the bounded feedback loop, at which point it performs an additional analysis of the accumulated failure evidence and makes one further repair attempt.
...
The repair backend is primarily driven by DeepSeek V4 Flash. Cases that still fail validation after the bounded repair loop are escalated to the vulnerability-domain model Zhian (Zhian-27B), which performs one targeted analysis and repair attempt using the accumulated failure evidence.
